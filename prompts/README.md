...
|---|---|---|---|
| `enrich-memory-prompt.md` | Phase 0 / 1 | Memory enrichment (why / strand / impact extraction from a To-Do task) | Not yet authored |
...
- Token / credential acquisition via `DefaultAzureCredential` with the `https://ai.azure.com/.default` scope.
- Schema validation of LLM output before persistence.
- Real calendar scheduling via Microsoft Graph `findMeetingTimes` and `POST /me/events`. The LLM proposes intent; Pensieve handles the calendar API.
- Cross-referencing returned IDs against the local SQLite store.
