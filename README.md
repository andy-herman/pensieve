...
- **Garden v1+v2 — board tending game.** Every card gets a freshness dot (fresh/stale/ghost) based on `last_tended_at`, the masthead shows a HEALTH 00–100 pill, and a daily-quest panel below the toolbar surfaces up to 3 actionable quests per day with a 🔥 clean-streak counter. Auto-sync intentionally does NOT bump tending — only deliberate user actions do, so the staleness signal is real. v3 (achievement badges + weekly level-summary) is next; see `brainstorms/02-board-tending-game.md`.
